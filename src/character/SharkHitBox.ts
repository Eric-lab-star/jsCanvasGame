...
export default class SharkHitBox {
  ...
  public moveShark(time: number = 0) {
    ...
    if (this.hitBox.body.position.x >= 440) {
      this.movingDirection = -1;
      Body.setVelocity(this.hitBox.body, { x: this.movingDirection, y: 0 });
    }
    if (this.hitBox.body.position.x <= 300) {
      this.movingDirection = 1;
      Body.setVelocity(this.hitBox.body, {
        x: this.movingDirection,
        y: 0,
    ...
    requestAnimationFrame((time) => this.moveShark(time));
  }
}
